...
def get_time_epoch(epoch, tz_from=pytz.utc, tz_to=pytz.timezone("Europe/London")):
    dt = datetime.datetime.fromtimestamp(epoch, tz_from)
    return dt.astimezone(tz_to)


def parse_datetime(datetime_string, tz_from=pytz.utc, tz_to=pytz.timezone("Europe/London")):
